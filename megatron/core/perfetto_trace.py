...
def _get_tracer() -> Optional[_Tracer]:
    ...
    with _init_lock:
        ...
        # Publish _init_done LAST: the lock-free fast path above reads it without
        # the lock, so it must only observe True once _tracer holds its final
        # value. Setting it earlier let concurrent threads (e.g. the dataset
        # builder's ThreadPoolExecutor) read _init_done=True while _tracer was
        # still None during the _Tracer(path) file-open window, silently
        # dropping every marker emitted from those threads.
        _init_done = True
        return _tracer
